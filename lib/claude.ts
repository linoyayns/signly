...
4. הסעיף "שיפוט" — בתי המשפט המוסמכים יהיו ב${getCourtCity(data.freelancerCity)}.
5. כלול סעיף חתימות עם שני חותמים — ערוך אותם זה לצד זה בפורמט טבלה בשורה אחת:
   | **חתימת נותן השירות:** _______________ שם: ${data.freelancerName} תאריך: _______________ | **חתימת הלקוח:** _______________ שם: ${data.clientName} תאריך: _______________ |
6. בשורת "נערך ביום" בראש החוזה ובשורות החתימה — השתמש בתאריך החתימה שצוין. אם לא צוין — הוסף קו תחתון (___________) למילוי ידני.
7. אם מצוין שהפרילנסר עוסק פטור — ציין זאת בסעיף התשלום והדגש שלא תצורף חשבונית מע"מ.`;
